<?php
    ...
    switch($_GET["op"])
    {
        case 'guardaryeditar':
            ...
            if (empty($idusuario)){
                $rspta=$usuario->insertar($nombre,$login,$clavehash,$imagen,$_POST['permiso'],$_POST['entidad']);
                echo $rspta ? "Usuario registrado" : "No se pudieron registrar todos los datos del usuario";
            }
            else {
                $rspta=$usuario->editar($idusuario,$nombre,$login,$clavehash,$imagen,$_POST['permiso'],$_POST['entidad']);
        ...
        case 'listar':
            ...
            while ($reg = $rspta->fetch_object()) {
                $data[] = array(
                    "0"=> ($reg->condicion) ? 
                        '<button class="btn btn-warning" onclick="mostrar('.$reg->idusuario.')" title="Editar Usuario"><li class="fa fa-pencil"></li></button>'.
                        ' <button class="btn btn-danger" onclick="desactivar('.$reg->idusuario.')" title="Desactivar Usuario"><li class="fa fa-close"></li></button>'
                        :
                        '<button class="btn btn-warning" onclick="mostrar('.$reg->idusuario.')" title="Editar Usuario"><li class="fa fa-pencil"></li></button>'.
                        ' <button class="btn btn-primary" onclick="activar('.$reg->idusuario.')" title="Activar Usuario"><li class="fa fa-check"></li></button>'
                        ,
                    "1"=>$reg->nombre,
                    "2"=>$reg->login,
        ...
        case 'entidades':
            ...
            //Obtener los permisos del usuario
            $id=$_GET['id'];
            $marcados = $usuario->listarEntidades($id);
            
            //declaramos el array para almacenar todos los permisos marcados
            $valores = array();

            //Almacenar los permisos asignados al usuario en el array
            while ($ent = $marcados->fetch_object()) 
            {
                array_push($valores,$ent->identidad);
            }

            while($reg = $rspta->fetch_object())
            {
                $sw = in_array($reg->identidad, $valores) ? 'checked' : '';

                echo '<li> 
                        <input type="checkbox" '.$sw.' name="entidad[]" value="'.$reg->identidad.'">'
                        .$reg->descripcion.
                    '</li>';
            }
        break;
        ...
        case 'verificar':
            ...
            if(isset($fetch))
            {
               ...
               /* in_array(1,$valores) ? $_SESSION['escritorio'] = 1 : $_SESSION['escritorio'] = 0;
                ...
                in_array(4,$valores) ? $_SESSION['consulta'] = 1 : $_SESSION['consulta'] = 0;*/
